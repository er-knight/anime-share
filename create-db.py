...
sqlitedb_path = Path(__file__).parent / 'backend' / 'anime.db'
with sqlite3.connect(str(sqlitedb_path)) as connection:
    ...
    def stage_two():
        compressed_files_dir = Path(__file__).parent / 'compressed-files' 
            ...
            if filepath.is_file():
                with lzma.open(filepath, 'r') as f:
                    ...
                    for row in soup.find_all('tr', {'class': 'ranking-list'}):
                        anime_id = int(row.find('a', {'class': 'hoverinfo_trigger'}).get('href').split('/')[4])
                        rank = (lambda s: int(s) if s.isdigit() else None)(
                            row.find('span', {'class': 'top-anime-rank-text'}).text
                        )
                        ...
                        if parts:
                            if len(parts) == 1:
                                if parts[0].isdigit():
                                    ...
                                    airing_period = datetime.strptime(parts[0], '%b %Y').strftime('%m/%Y')
                            elif len(parts) == 2:
                                if parts[0].isdigit() and parts[1].isdigit():
                                    ...
                                    parts[0] = datetime.strptime(parts[0], '%b %Y').strftime('%m/%Y')
                                    parts[1] = datetime.strptime(parts[1], '%b %Y').strftime('%m/%Y')
                                    if parts[0] == parts[1]:
                                        airing_period = parts[0]
                                    else:
                                        airing_period = f'{parts[0]} - {parts[1]}'
